...
You can run any command automatically, but make sure you verify with me the changes you are about to make before making them.

Whenever ssh'ing, use the -A flag to forward the agent.

When testing code that can be cloned, always pull the latest version on the test system, don't scp one-offs over.

Always run AWS CLI commands with `AWS_DEFAULT_PROFILE=arm`.
Perform all AWS actions on us-east-1 unless I specify otherwise.

Prefix all AWS instances with "gcohen-".
